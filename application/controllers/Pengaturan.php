...
class Pengaturan extends CI_Controller {
	...
	public function pengguna($filter1='', $filter2='', $filter3='')
	{
		if ($this->session->userdata('logged_in') == TRUE){
			...
			if ($data['action'] == 'view'){
				$data['berdasarkan']		= array('admin_user'=>'NOMBRE DE USUARIO',
													'admin_nama'=>'NOMBRE COMPLETO',
													'admin_telepon'=>'TELÉFONO',
													'admin_level_kode'=>'GRUPO');
				...
				if ($simpan){
					...
					// Verificar si se proporcionó una nueva contraseña en el formulario y si es diferente de la contraseña actual
					$new_pass = $this->input->post('admin_pass');
					if (!empty($new_pass) && $new_pass != $admin->admin_pass) {
						// Actualizar la contraseña solo si se proporciona una nueva contraseña y es diferente de la contraseña actual
						$edit['admin_pass'] = validasi_sql(do_hash($new_pass, 'md5'));
					} else {
						// Si no se proporciona una nueva contraseña o es igual a la contraseña actual, no actualizar la contraseña
						$edit['admin_pass'] = $admin->admin_pass;
					}
