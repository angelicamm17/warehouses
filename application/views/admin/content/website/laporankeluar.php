...
<script>
    ...
    subtitle: {
        text: 'Estadísticas de Productos Salientes'
    },
    xAxis: {
        categories: [
			<?php 
    ...
    yAxis: {
        min: 0,
        title: {
            text: 'Número de Productos Salientes (pcs)'
        }
    },
    tooltip: {
    ...
    series: [{
        name: 'Total de Productos Salientes',
        data: [
			<?php 
			$where_transaksi['status_pergerakan'] 	= 2;
			foreach ($this->ADM->grid_all_transaksi('', 'id_transaksi', 'DESC', 1000, '', $where_transaksi, '') as $row){ ?>
			...
			<?php } ?>
                ...
                </div><a href="<?php echo base_url();?>website/laporankeluarpdf" class="btn btn-success btn-block btn-lg" style="margin-top: 30px !important;" target="_blank">Descaragar PDF de Reporte de Mercancía Saliente</a>
            </div>
        </div>
    </div>
    </div>
